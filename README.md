...
src/
  ...
  tools/
    ...
    weather-watch.ts        инструменты 5-8: периодический сбор
test/
  unit.test.ts              конфиг, клиент, декодеры (30 тестов)
  integration.test.ts       полный round-trip через MCP-клиент (39 тестов)
  watch.test.ts             агрегация, хранилище, retention, права (25 тестов)
...
```bash
npm test          # 94 теста: unit + integration + watch, полностью герметично
npm run typecheck
```
