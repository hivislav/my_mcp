...
- Исходящий HTTPS к `api.open-meteo.com`, `geocoding-api.open-meteo.com`, `air-quality-api.open-meteo.com`.
...
Прогноз по дням до 16 суток: min/max температура, «ощущается», сумма и вероятность осадков, снег, УФ-индекс, восход/закат, максимум ветра. Опционально — почасовой ряд.
...
- **Ошибки приходят как `isError: true` с текстом**, а не как обрыв транспорта: упоминается вид ошибки, HTTP-статус, причина от upstream и то, временная она или нет. Пример:
  `Open-Meteo request failed [network]: Could not reach api.open-meteo.com: fetch failed. This looks transient — retrying may succeed.`
...
| `OPEN_METEO_*_URL` | официальные | Переопределение базовых URL (свой инстанс/прокси). |
...
test/
  unit.test.ts              конфиг, клиент, декодеры (25 тестов)
  integration.test.ts       полный round-trip через MCP-клиент (24 теста)
...
```bash
npm test          # 49 тестов: unit + integration, полностью герметично
npm run typecheck
```
...
- **`include_hourly` на 16 дней** даёт ~384 объекта — большой ответ; включайте осознанно.
